...
output$the_pcs_to_plot_y <- renderUI({
  ...
  output$plot2 <- renderPlot({
    ...
    ggplot(eig_df, aes(reorder(PCs, -eig), eig)) +
      geom_bar(stat = "identity", aes(fill=PCs), colour = "black",show.legend = FALSE) +
 ...
 pca_biplot2 <- reactive({

   pca_output <-  pca_objects()$pca_output
   pca_output.n <- pca_objects()$pca_output$x
   ...
   ggbiplot::ggbiplot(
     pca_output,
     choices = which(colnames(pca_output.n)  %in% c(input$the_pcs_to_plot_x, input$the_pcs_to_plot_y)),
     obs.scale = 1,
     var.scale = 1,
	 varname.size=5,
     ellipse = TRUE,
